...
Welcome to the `agro` Vertical Sector.
...
This is a monorepo that contains the three core components of the `agro` sector:
...
This sector *consumes* services from Horizontal Enablers like `hub`, `finance`, and `balance`.
...
3.  Clone this repository: `git clone https://github.com/nova-ecosystem/agro.git`
4.  Open the cloned folder in VS Code.
5.  A pop-up will appear: "Folder contains a Dev Container... Reopen in Container?". Click **"Reopen in Container"**.

This will instantly download the pre-built `dev-python` and `dev-node` images and start all three `agro` services (`api`, `app`, `website`) in an integrated environment.

**Note:** This environment does *not* run other enablers (like `hub`). You must write unit tests in `/api/tests` that *mock* any external API calls.